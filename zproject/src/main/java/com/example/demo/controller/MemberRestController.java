...
public class MemberRestController {
	...
	public ResponseEntity<String> changeEmail(@NotEmpty String email, Principal principal) {
		Boolean result = service.changeEmail(email, principal.getName());
		System.out.println(result);
		if(result)
			return ResponseEntity.ok("이메일을 변경했습니다");
		return ResponseEntity.status(HttpStatus.CONFLICT).body("이메일을 변경하지 못했습니다");
	}
